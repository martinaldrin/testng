 ...
 * consist of a single test class while the rest shall consist of more than one test class.
 *
 * <p>1) The suite thread pool is 2, so neither suite should have to wait for the other to complete
 * execution 2) For both suites, the thread count and parallel mode are specified at the suite level
 * 3) The thread count is less than the number of test methods in a test for one of the suites, so
 * some methods will have to wait the active thread count to drop below the maximum thread count
 * before they can begin execution. 4) The thread count is more than the number of test methods for
 * the rest of the tests, ensuring that none of their test methods should have to wait for any other
 * method to complete execution 5) There are NO configuration methods 6) All test methods pass 7) NO
 * ordering is specified 8) group-by-instances is NOT set 9) There are no method exclusions
 */
public class ParallelByMethodsTestCase2Scenario1 extends BaseParallelizationTest {
  private static final Logger log = Logger.getLogger(ParallelByMethodsTestCase2Scenario1.class);
  ...
  public void setUp() {
    ...
    log.debug(
        "Beginning ParallelByMethodsTestCase2Scenario1. This test scenario consists of two "
  ...
  public void verifyThatTestMethodsRunInParallelThreads() {
    ...
    verifySimultaneousTestMethods(
        getTestMethodLevelEventLogsForTest(SUITE_B, SUITE_B_TEST_A), SUITE_B_TEST_A, 14);
    verifySimultaneousTestMethods(
        getTestMethodLevelEventLogsForTest(SUITE_B, SUITE_B_TEST_B), SUITE_B_TEST_B, 14);
  }
}
