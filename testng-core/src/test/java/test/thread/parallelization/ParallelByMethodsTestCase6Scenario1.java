 ...
 *
 * <p>Test Case Summary: Parallel by methods mode with parallel test suites using factories but no
 ...
 * consist of factories which provide multiple instances of multiple test classes. One suite shall
 * consist of a single test with multiple test classes which uses factories. One suite shall have
 * multiple tests with multiple classes, none of which use a factory.
 *
 ...
 * NO ordering is specified 11) group-by-instances is NOT set 12) There are no method exclusions
 */
public class ParallelByMethodsTestCase6Scenario1 extends BaseParallelizationTest {
  ...
  public void setUp() {
    ...
    createXmlTest(
        suiteOne,
        SUITE_A_TEST_A,
        FactoryForTestClassAFiveMethodsWithNoDepsTwoInstancesSample.class,
    ...
    log.debug(
        "Beginning ParallelByMethodsTestCase6Scenario1. This test scenario consists of three "
    ...
    log.debug(
        "Suite: "
            ...
            + ", "
            + TestClassCSixMethodsWithNoDepsSample.class.getCanonicalName()
            + ". Thread count: 10");
  ...
  public void sanityCheck() {
    ...
    assertEquals(
        ...
        84,
        "There should be 84 test method level events "
            + "logged for "
            + SUITE_A
            + ": "
    ...
    assertEquals(
        ...
        219,
        "There should be 219 test method level events "
            + "logged for "
            + SUITE_B
            + ": "
    ...
    assertEquals(
        ...
        117,
        "There should be 87 test method level events "
            + "logged for "
            + SUITE_C
            + ": "
  ...
  public void verifyThatMethodLevelEventsRunInDifferentThreadsFromSuiteAndTestLevelEvents() {
    ...
    verifyEventsForTestMethodsRunInTheSameThread(
        TestClassAFiveMethodsWithNoDepsSample.class, SUITE_A, SUITE_A_TEST_A);
    verifyEventsForTestMethodsRunInTheSameThread(
        TestClassCSixMethodsWithNoDepsSample.class, SUITE_A, SUITE_A_TEST_A);
    ...
    verifyEventsForTestMethodsRunInTheSameThread(
        TestClassBFourMethodsWithNoDepsSample.class, SUITE_B, SUITE_B_TEST_B);
    verifyEventsForTestMethodsRunInTheSameThread(
        TestClassFSixMethodsWithNoDepsSample.class, SUITE_B, SUITE_B_TEST_B);
  ...
  // Verify that the methods are run in separate threads in true parallel fashion by checking that
  // the start and run
  // times of events that should be run simultaneously start basically at the same time using the
  // timestamps and the
  // known values of the wait time specified for the event. Verify that the thread IDs of parallel
  // events are
  // different.
  @Test
  public void verifyThatTestMethodsRunInParallelThreads() {
    ...
    verifySimultaneousTestMethods(
        getTestMethodLevelEventLogsForTest(SUITE_C, SUITE_C_TEST_A), SUITE_C_TEST_A, 10);
    verifySimultaneousTestMethods(
