 ...
 *
 * <p>Test Case Summary: Parallel by methods mode with parallel test suites using factories with
 * data providers but no dependencies.
 *
 * <p>Scenario Description: Three suites with 1, 2 and 3 tests respectively. One suite with two
 * tests has a test consisting of a single test class without a factory while the other shall
 * consist of test classes with factories using data providers with varying numbers of data sets.
 * One suite shall consist of a single test with multiple test classes with factories using data
 * providers with varying numbers of data sets. One suite shall have multiple tests with multiple
 * classes, none of which use a factory.
 *
 ...
 * tests in two of the suites, so some methods will have to wait for the active thread count to drop
 * below the maximum thread count before they can begin execution. 6) The thread count is more than
 * the number of test methods for the tests in one of the suites, ensuring that none of the methods
 * in that suite should have to wait for any other method to complete execution 7) The expectation
 * is that threads will be spawned for each test method that executes just as they would if there
 * were no factories and test suites simply used the default mechanism for instantiating single
 * instances of the test classes. 8) There are NO configuration methods 9) All test methods pass 10)
 * NO ordering is specified 11) `group-by-instances is NOT set 12) here are no method exclusions
 */
public class ParallelByMethodsTestCase8Scenario1 extends BaseParallelizationTest {
  ...
  public void setUp() {
    ...
    createXmlTest(
        suiteThree,
        SUITE_C_TEST_B,
        TestClassJFourMethodsWithNoDepsSample.class,
    ...
    log.debug(
        "Beginning ParallelByMethodsTestCase8Scenario1. This test scenario consists of three "
    ...
    log.debug(
        "Suite: "
            ...
            + SUITE_B_TEST_A
            + ", Test class: "
            + TestClassEFiveMethodsWithNoDepsSample.class.getCanonicalName()
            + ". Thread count: 3");
    ...
    expectedInvocationCounts.put(
        TestClassGThreeMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodA", 1);
    expectedInvocationCounts.put(
        TestClassGThreeMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodB", 1);
    expectedInvocationCounts.put(
        TestClassGThreeMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodC", 1);

    expectedInvocationCounts.put(
        TestClassHFourMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodA", 1);
    expectedInvocationCounts.put(
        TestClassHFourMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodB", 1);
    expectedInvocationCounts.put(
        TestClassHFourMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodC", 1);
    expectedInvocationCounts.put(
        TestClassHFourMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodD", 1);

    expectedInvocationCounts.put(
        TestClassIFiveMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodA", 1);
    expectedInvocationCounts.put(
        TestClassIFiveMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodB", 1);
    expectedInvocationCounts.put(
        TestClassIFiveMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodC", 1);
    expectedInvocationCounts.put(
        TestClassIFiveMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodD", 1);
    expectedInvocationCounts.put(
        TestClassIFiveMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodE", 1);

    expectedInvocationCounts.put(
        TestClassJFourMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodA", 1);
    expectedInvocationCounts.put(
        TestClassJFourMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodB", 1);
    expectedInvocationCounts.put(
        TestClassJFourMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodC", 1);
    expectedInvocationCounts.put(
        TestClassJFourMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodD", 1);

    expectedInvocationCounts.put(
        TestClassKFiveMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodA", 1);
    expectedInvocationCounts.put(
        TestClassKFiveMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodB", 1);
    expectedInvocationCounts.put(
        TestClassKFiveMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodC", 1);
    expectedInvocationCounts.put(
        TestClassKFiveMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodD", 1);
    expectedInvocationCounts.put(
        TestClassKFiveMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodE", 1);

    expectedInvocationCounts.put(
        TestClassLThreeMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodA", 1);
    expectedInvocationCounts.put(
        TestClassLThreeMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodB", 1);
    expectedInvocationCounts.put(
        TestClassLThreeMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodC", 1);

    expectedInvocationCounts.put(
        TestClassMFourMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodA", 1);
    expectedInvocationCounts.put(
        TestClassMFourMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodB", 1);
    expectedInvocationCounts.put(
        TestClassMFourMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodC", 1);
    expectedInvocationCounts.put(
        TestClassMFourMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodD", 1);

    expectedInvocationCounts.put(
        TestClassNFiveMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodA", 1);
    expectedInvocationCounts.put(
        TestClassNFiveMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodB", 1);
    expectedInvocationCounts.put(
        TestClassNFiveMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodC", 1);
    expectedInvocationCounts.put(
        TestClassNFiveMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodD", 1);
    expectedInvocationCounts.put(
        TestClassNFiveMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodE", 1);

    expectedInvocationCounts.put(
        TestClassOSixMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodA", 1);
    expectedInvocationCounts.put(
        TestClassOSixMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodB", 1);
    expectedInvocationCounts.put(
        TestClassOSixMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodC", 1);
    expectedInvocationCounts.put(
        TestClassOSixMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodD", 1);
    expectedInvocationCounts.put(
        TestClassOSixMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodE", 1);
    expectedInvocationCounts.put(
        TestClassOSixMethodsWithNoDepsSample.class.getCanonicalName() + ".testMethodF", 1);
  ...
  public void verifyNumberOfInstanceOfTestClassForAllTestMethodsForAllSuites() {
    ...
    verifyNumberOfInstancesOfTestClassForMethods(
        SUITE_B, SUITE_B_TEST_A, TestClassEFiveMethodsWithNoDepsSample.class, 1);

    verifySameInstancesOfTestClassAssociatedWithMethods(
        SUITE_B, SUITE_B_TEST_A, TestClassEFiveMethodsWithNoDepsSample.class);
  ...
  public void verifyThatMethodLevelEventsRunInDifferentThreadsFromSuiteAndTestLevelEvents() {
    ...
    verifyEventsForTestMethodsRunInTheSameThread(
        TestClassEFiveMethodsWithNoDepsSample.class, SUITE_B, SUITE_B_TEST_A);
    ...
    verifyEventsForTestMethodsRunInTheSameThread(
        TestClassGThreeMethodsWithNoDepsSample.class, SUITE_C, SUITE_C_TEST_A);
    verifyEventsForTestMethodsRunInTheSameThread(
        TestClassHFourMethodsWithNoDepsSample.class, SUITE_C, SUITE_C_TEST_A);
    verifyEventsForTestMethodsRunInTheSameThread(
        TestClassIFiveMethodsWithNoDepsSample.class, SUITE_C, SUITE_C_TEST_A);

    verifyEventsForTestMethodsRunInTheSameThread(
        TestClassJFourMethodsWithNoDepsSample.class, SUITE_C, SUITE_C_TEST_B);
    verifyEventsForTestMethodsRunInTheSameThread(
        TestClassKFiveMethodsWithNoDepsSample.class, SUITE_C, SUITE_C_TEST_B);

    verifyEventsForTestMethodsRunInTheSameThread(
        TestClassLThreeMethodsWithNoDepsSample.class, SUITE_C, SUITE_C_TEST_C);
    verifyEventsForTestMethodsRunInTheSameThread(
        TestClassMFourMethodsWithNoDepsSample.class, SUITE_C, SUITE_C_TEST_C);
    verifyEventsForTestMethodsRunInTheSameThread(
        TestClassNFiveMethodsWithNoDepsSample.class, SUITE_C, SUITE_C_TEST_C);
    verifyEventsForTestMethodsRunInTheSameThread(
        TestClassOSixMethodsWithNoDepsSample.class, SUITE_C, SUITE_C_TEST_C);
  }
  ...
  public void verifyThatTestMethodsRunInParallelThreads() {
    verifySimultaneousTestMethods(
        getTestMethodLevelEventLogsForTest(SUITE_A, SUITE_A_TEST_A), SUITE_A_TEST_A, 10);
    verifySimultaneousTestMethods(
        getTestMethodLevelEventLogsForTest(SUITE_B, SUITE_B_TEST_A), SUITE_B_TEST_A, 3);
    verifySimultaneousTestMethods(
        getTestMethodLevelEventLogsForTest(SUITE_B, SUITE_B_TEST_B), SUITE_B_TEST_B, 20);
    verifySimultaneousTestMethods(
        getTestMethodLevelEventLogsForTest(SUITE_C, SUITE_C_TEST_A), SUITE_C_TEST_A, 25);
    verifySimultaneousTestMethods(
        getTestMethodLevelEventLogsForTest(SUITE_C, SUITE_C_TEST_B), SUITE_C_TEST_B, 5);
    verifySimultaneousTestMethods(
        getTestMethodLevelEventLogsForTest(SUITE_C, SUITE_C_TEST_C), SUITE_C_TEST_C, 12);
  }
}
