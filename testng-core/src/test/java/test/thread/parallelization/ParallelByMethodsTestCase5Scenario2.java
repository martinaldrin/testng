 ...
 *
 * <p>Scenario Description: Two suites with 1 and 2 tests respectively. One suite with two tests has
 * a test consisting of a single test class without a factory while the other consists of factories
 * which provide multiple instances of multiple test classes. One suite shall consist of a single
 * test with multiple test classes which uses factories.
 *
 ...
 * will have to wait for the active thread count to drop below the maximum thread count before they
 * can begin execution. The expectation is that threads will be spawned for each test method that
 * executes just as they would if there were no factories and test suites simply used the default
 * mechanism for instantiating single instances of the test classes. 3) There are NO configuration
 * methods 4) All test methods pass 5) NO ordering is specified 6) group-by-instances is NOT set 7)
 * There are no method exclusions
 */
public class ParallelByMethodsTestCase5Scenario2 extends BaseParallelizationTest {
  private static final Logger log = Logger.getLogger(ParallelByMethodsTestCase5Scenario2.class);
  ...
  public void setUp() {
    ...
    log.debug(
        "Beginning ParallelByMethodsTestCase5Scenario2. This test scenario consists of two "
  ...
  public void sanityCheck() {
    ...
    assertEquals(
        ...
        303,
        "There should 303 test method level events logged for "
            + SUITE_A
            + " and "
            + SUITE_B
            + ": "
  ...
  // Verify that all the events in the second suite run have timestamps later than the suite
  // listener's onFinish event
