 ...
 *
 * <p>Test Case Summary: Parallel by methods mode with sequential test suites using a factory which
 * uses a non-parallel data provider and there are no dependencies.
 *
 * <p>Scenario Description: Single suite with a single test consisting of a single test class with
 * five methods with a factory method using a data provider specifying 3 sets of data.
 *
 ...
 * a data set from the data provider. Expectation is that only 15 threads will be spawned at once.
 * There will be three instances of the test class, one for each of the three data sets and each
 * instance will have the five test methods defined on the class, for a total of 15 test method
 * invocations. 3) No test method invocation should be queued because the thread count is sufficient
 * to service all the test method invocations. 4) There are NO configuration methods 5) All test
 * methods pass 6) NO ordering is specified 7) group-by-instances is NOT set 8) There are no method
 * exclusions
 */
public class ParallelByMethodsTestCase7Scenario1 extends BaseParallelizationTest {
  ...
  public void setUp() {
    ...
    createXmlTest(
        suite, TEST, TestClassAFiveMethodsWithFactoryUsingDataProviderAndNoDepsSample.class);
    ...
    log.debug(
        "Beginning ParallelByMethodsTestCase7Scenario1. This test scenario consists of a "
  ...
  // Verify that there are three test class instance associated with each of the test methods from
  // the sample test
  // class
  ...
  public void verifyThreeInstancesOfTestClassForAllTestMethods() {
    ...
    verifyNumberOfInstancesOfTestClassForMethods(
        SUITE, TEST, TestClassAFiveMethodsWithFactoryUsingDataProviderAndNoDepsSample.class, 3);
  ...
  // Verifies that all the test method level events for any given test method in any given instance
  // run in the same
  // thread.
  @Test
