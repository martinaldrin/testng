 ...
 *
 * <p>Test Case Summary: Parallel by methods mode with sequential test suites using a factory with a
 * non-parallel data provider and there are no dependencies.
 *
 ...
 * using data providers with varying numbers of data sets which provide multiple instances of
 * multiple test classes. One suite shall consist of a single test with multiple test classes which
 * use factories with data providers with varying numbers of data sets.
 *
 ...
 * executes just as they would if there were no factories and data providers and test suites simply
 * use the default mechanism for instantiating single instances of the test classes. 3) There are NO
...
public class ParallelByMethodsTestCase7Scenario2 extends BaseParallelizationTest {
  private static final Logger log = Logger.getLogger(ParallelByMethodsTestCase7Scenario2.class);
  ...
  public void setUp() {
    ...
    log.debug(
        "Beginning ParallelByMethodsTestCase7Scenario2. This test scenario consists of two "
  ...
  public void sanityCheck() {
    ...
    assertEquals(
        ...
        297,
        "There should 267 test method level events logged for "
            + SUITE_A
            + " and "
            + SUITE_B
            + ": "
    ...
    assertEquals(
        ...
        180,
        "There should be 180 test method level events "
            + "logged for "
            + SUITE_B
            + ": "
  ...
  public void verifyThatTestMethodsRunInParallelThreads() {
    verifySimultaneousTestMethods(
        getTestMethodLevelEventLogsForTest(SUITE_A, SUITE_A_TEST_A), SUITE_A_TEST_A, 25);
    verifySimultaneousTestMethods(
        getTestMethodLevelEventLogsForTest(SUITE_B, SUITE_B_TEST_A), SUITE_B_TEST_A, 3);
    verifySimultaneousTestMethods(
        getTestMethodLevelEventLogsForTest(SUITE_B, SUITE_B_TEST_B), SUITE_B_TEST_B, 40);
  }
}
